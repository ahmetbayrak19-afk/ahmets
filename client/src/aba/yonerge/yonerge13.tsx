...
const TRIALS: Trial[] = [
  { id: 't1', type: 'candle', text: 'Yanan eli kurtar! Salla veya üfle, mumu söndür' },
...
function LiveCandle({ onExtinguish, onFail, active }: { onExtinguish: () => void; onFail: () => void; active: boolean }) {
    ...
    const startMic = async () => {
      try {
        ...
        const tick = () => {
          ...
          const isBlow = rms > 0.05 && zcr > 0.12 && highRatio > 0.42 && high > 0.04;
          if (isBlow) {
            blowAccum.current += rms * 1.5 + high * 0.8;
            ...
            blowAccum.current = Math.max(0, blowAccum.current * 0.78);
          }
          if (blowAccum.current > 2.8) doExtinguish();
  ...
  return (
    <div className="relative w-full max-w-sm mx-auto aspect-[3/4] max-h-[52vh]">
      ...
      <div className="absolute z-10" style={{ left: '12%', top: '61%', transform: 'translate(-50%, 0)' }}>
