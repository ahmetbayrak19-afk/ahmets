...
export default function Yonerge12({
  ...
  const finishTrial = useCallback(
    async (correct: boolean) => {
      ...
      setLocked(true);
      setDrawingId(null);
      // Son adım zaten onay sesi çaldı (advance içinde).
      // 3lü bitti → nötr ses çal, bitmesini bekle, sonra sonraki yönergeye geç.
  ...
  }, [phase, locked, expectedKind, expected, advance, seqPos, idx]);
...
function SwipeTile({
  ...
  return (
    <div
      ...
      style={{ transform: 'translateX(' + Math.max(-100, Math.min(100, dx)) + 'px)' }}
      onPointerDown={(e) => {
        if (disabled || done) return;
        e.preventDefault();
      ...
      onPointerMove={(e) => {
        if (disabled || done) return;
        const d = e.clientX - sx.current;
        dxRef.current = d;
        setDx(d);
      }}
      onPointerUp={() => {
        // dx state stale olabilir; canlı mesafe için ref kullan
        if (Math.abs(dxRef.current) > 100 && isExpected) {
          onSwipe();
        }
        dxRef.current = 0;
        setDx(0);
