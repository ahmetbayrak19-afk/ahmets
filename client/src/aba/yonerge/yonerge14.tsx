...
/** 12 dijital yönerge havuzu — her seferinde rastgele 5 seçilir */
const DIGITAL_POOL: DigitalTask[] = [
  {
    id: 'd1', kind: 'connect', text: 'Mavi noktayı kırmızı noktaya birleştir',
    points: [{ x: 0.18, y: 0.5 }, { x: 0.82, y: 0.5 }],
    colors: ['#3b82f6', '#ef4444'], labels: ['Mavi', 'Kırmızı'],
  },
  {
    id: 'd2', kind: 'connect', text: 'Yeşil noktayı sarı noktaya birleştir',
    points: [{ x: 0.2, y: 0.22 }, { x: 0.8, y: 0.78 }],
    colors: ['#22c55e', '#eab308'], labels: ['Yeşil', 'Sarı'],
  },
  {
    id: 'd3', kind: 'connect', text: 'Mor noktayı turuncu noktaya birleştir',
    points: [{ x: 0.75, y: 0.2 }, { x: 0.25, y: 0.8 }],
    colors: ['#a855f7', '#f97316'], labels: ['Mor', 'Turuncu'],
  },
  {
    id: 'd4', kind: 'path', text: 'Yolu çizerek baştan sona git',
    path: [
      { x: 0.12, y: 0.55 }, { x: 0.28, y: 0.35 }, { x: 0.45, y: 0.55 },
      { x: 0.62, y: 0.35 }, { x: 0.78, y: 0.55 }, { x: 0.9, y: 0.4 },
    ],
  },
  {
    id: 'd5', kind: 'path', text: 'Kavisli yolu takip et',
    path: [
      { x: 0.1, y: 0.7 }, { x: 0.25, y: 0.3 }, { x: 0.45, y: 0.65 },
      { x: 0.65, y: 0.28 }, { x: 0.88, y: 0.55 },
    ],
  },
  {
    id: 'd6', kind: 'sequence', text: 'Noktaları sırayla birleştir: 1 → 2 → 3',
    points: [{ x: 0.2, y: 0.7 }, { x: 0.5, y: 0.25 }, { x: 0.8, y: 0.7 }],
    colors: ['#38bdf8', '#38bdf8', '#38bdf8'], labels: ['1', '2', '3'],
  },
  {
    id: 'd7', kind: 'sequence', text: 'Noktaları sırayla birleştir: 1 → 2 → 3 → 4',
    points: [
      { x: 0.2, y: 0.25 }, { x: 0.8, y: 0.25 },
      { x: 0.8, y: 0.75 }, { x: 0.2, y: 0.75 },
    ],
    colors: ['#38bdf8', '#38bdf8', '#38bdf8', '#38bdf8'],
    labels: ['1', '2', '3', '4'],
  },
  {
    id: 'd8', kind: 'shape', text: 'Kare çiz (köşeleri birleştir)',
    shape: 'square',
    points: [
      { x: 0.28, y: 0.22 }, { x: 0.72, y: 0.22 },
      { x: 0.72, y: 0.72 }, { x: 0.28, y: 0.72 },
    ],
  },
  {
    id: 'd9', kind: 'shape', text: 'Üçgen çiz (köşeleri birleştir)',
    shape: 'triangle',
    points: [
      { x: 0.5, y: 0.2 }, { x: 0.8, y: 0.75 }, { x: 0.2, y: 0.75 },
    ],
  },
  {
    id: 'd10', kind: 'connect', text: 'Üstteki noktayı alttaki noktaya birleştir',
    points: [{ x: 0.5, y: 0.18 }, { x: 0.5, y: 0.82 }],
    colors: ['#f472b6', '#2dd4bf'], labels: ['Üst', 'Alt'],
  },
  {
    id: 'd11', kind: 'path', text: 'Z şeklinde yolu çiz',
    path: [
      { x: 0.15, y: 0.22 }, { x: 0.85, y: 0.22 },
      { x: 0.15, y: 0.78 }, { x: 0.85, y: 0.78 },
    ],
  },
  {
    id: 'd12', kind: 'sequence', text: 'Noktaları sırayla birleştir: A → B → C',
    points: [{ x: 0.18, y: 0.35 }, { x: 0.5, y: 0.7 }, { x: 0.82, y: 0.35 }],
    colors: ['#fb7185', '#fb7185', '#fb7185'], labels: ['A', 'B', 'C'],
  },
];
...
/** İleri kazanım: daha hassas ama milimetrik değil (~%9 tolerans) */
...
function DrawCanvas({
  task, locked, onSuccess, onFail,
}: {
  task: DigitalTask; locked: boolean; onSuccess: () => void; onFail: () => void;
}) {
  ...
  const redraw = useCallback(() => {
    ...
    // clear in device pixels via reset transform handled by resize
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 w-full">
      ...
      <div className="shrink-0 flex items-center justify-center gap-3 py-2 px-3">
        <button type="button" disabled={locked || !hasStroke} onClick={clear}
          className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-slate-800 border border-slate-600 text-slate-300 text-xs font-bold disabled:opacity-40">
          <Eraser className="w-4 h-4" /> Temizle
        </button>
        <button type="button" disabled={locked} onClick={onFail}
          className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 text-xs font-bold disabled:opacity-40">
        ...
        <>
          <div className="shrink-0 px-4 pt-3 pb-2 text-center">
            <p className="text-[10px] text-slate-500 font-bold uppercase mb-1">
              {trial.mode === 'digital' ? 'Dijital çizim' : 'Öğretmen · kağıt-kalem'}
            </p>
            ...
            {trial.mode === 'digital' ? (
              ...
              <div className="flex-1 flex items-center justify-center px-6">
                <p className="text-lg text-slate-300 text-center font-medium">
                  Öğrenci kağıt-kalem ile yapsın.<br />
                  <span className="text-sm text-slate-500">Aşağıdan işaretleyin.</span>
                </p>
              </div>
            )}
          </div>
          ...
          <div className="shrink-0 p-3 pb-5 border-t border-slate-800 bg-slate-900/95 flex gap-3 justify-center items-center">
            {trial.mode === 'digital' && (
              <button type="button" disabled={locked} onClick={swapDigital}
            ...
            {trial.mode === 'teacher' && (
              <>
                <button type="button" disabled={locked} onClick={() => finishTrial(false)}
                ...
                <button type="button" disabled={locked} onClick={() => finishTrial(true)}
