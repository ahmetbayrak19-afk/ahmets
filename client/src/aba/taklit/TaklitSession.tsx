...
import { XCircle, ChevronLeft, ChevronRight, Check, X, Trophy, RefreshCcw } from 'lucide-react';
import { twMerge } from 'tailwind-merge';
import confetti from 'canvas-confetti';

// --- NESNELİ TAKLİT VİDEOLARI (Aynı klasörde oldukları için yollar kısaldı) ---
...
export default function TaklitSession({ mode, itemText, onClose, onSaveStatus }: TaklitSessionProps) {
  ...
  const activePlaylist = useMemo(() => {
    ...
    if (text.includes("NESNELİ") || text.includes("NESNE İLE")) return PLAYLISTS.NESNELI;
    if (text.includes("NESNESİZ") || text.includes("KABA MOTOR")) return PLAYLISTS.NESNESIZ;
    if (text.includes("YÜZ") || text.includes("DUDAK") || text.includes("MİMİK")) return PLAYLISTS.YUZDUDAK;
  ...
  return (
    <div className="fixed inset-0 h-[100dvh] w-screen z-[100] flex flex-col bg-slate-950 text-white">
      <div className="p-4 flex items-center justify-between border-b border-slate-800 bg-slate-900/50 backdrop-blur-md relative z-10">
        <button onClick={onClose} className="p-2 hover:bg-slate-800 rounded-full transition-colors text-slate-400 hover:text-white">
          <XCircle size={28} />
        </button>
        <div className="text-center">
          <h2 className="text-lg font-bold truncate max-w-[200px] sm:max-w-md">{currentVideo.name}</h2>
      ...
      <div className="flex-1 relative flex items-center justify-center overflow-hidden">
        {phase === 'playing' ? (
          ...
          <div className="flex flex-col items-center justify-center text-center p-8 bg-slate-900 rounded-3xl border border-slate-800 shadow-2xl">
            <Trophy size={80} className={score >= Math.ceil(activePlaylist.length / 2) ? "text-yellow-500 mb-6" : "text-slate-500 mb-6"} />
            ...
            <button onClick={completeSession} className="bg-blue-600 hover:bg-blue-500 text-white px-10 py-4 rounded-xl font-bold text-lg transition-all active:scale-95">
              KAYDET VE ÇIK
            </button>
          </div>
        )}
      ...
      {phase === 'playing' && (
        <div className="p-6 pb-10 bg-slate-900 border-t border-slate-800 flex items-center justify-center gap-6 relative z-10">
          {mode === 'instruction' ? (
            <>
              <button onClick={handlePrev} className="flex flex-col items-center gap-2 p-4 bg-slate-800 rounded-2xl active:scale-95 transition-all text-slate-300 hover:bg-slate-700 hover:text-white w-32">
                <ChevronLeft size={32} />
                <span className="text-xs font-bold uppercase tracking-wider">Önceki</span>
              </button>
              <button onClick={handleNext} className="flex flex-col items-center gap-2 p-4 bg-blue-600 rounded-2xl active:scale-95 transition-all text-white hover:bg-blue-500 w-32 shadow-lg shadow-blue-900/50">
            ...
            <>
              <button onClick={() => handleAssess(false)} className="flex flex-col items-center gap-2 p-4 bg-red-500/10 border-2 border-red-500/50 rounded-2xl active:scale-95 transition-all text-red-500 hover:bg-red-500/20 w-32">
                <X size={32} />
                <span className="text-xs font-bold uppercase tracking-wider">Yapamadı</span>
              </button>
              <button onClick={() => handleAssess(true)} className="flex flex-col items-center gap-2 p-4 bg-green-500 rounded-2xl active:scale-95 transition-all text-white hover:bg-green-400 w-32 shadow-lg shadow-green-900/50">
                <Check size={32} />
                <span className="text-xs font-bold uppercase tracking-wider">Yaptı</span>
              </button>
            </>
          )}
        </div>
