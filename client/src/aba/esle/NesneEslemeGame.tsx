...
export default function NesneEslemeGame({ mode, gameType, onClose, onComplete }: GameProps) {
  ...
  const runModelingDemo = () => {
    setIsModeling(true);
    // HIZLANDIRILDI: 2.5 Saniye (Eskiden 4'tü)
  ...
  const handleMistake = () => {
    if (mode === 'assessment') {
        ...
        setShowFeedback('wrong'); // Kırmızı Çarpı

        setTimeout(() => { 
            setShowFeedback(null); // Çarpı Gizle

            // 1. Yanlışsa -> Modelleme (İpucu) Başlat
            if (newCount === 1) {
                runModelingDemo();
            }
            // 2. Yanlışta -> Render içinde kilitlenecek
        }, 1000);
    }
  };
  ...
  const handleSuccess = () => {
    ...
    setTimeout(() => {
      if (mode === 'assessment') {
        const next = assessmentCount + 1; setAssessmentCount(next);
        if (next < 10) generateQuestion(); else { setPhase(assessmentScore >= 8 ? 'success' : 'fail'); if(assessmentScore>=8) try { confetti(); } catch {} }
      ...
      {phase === 'playing' && (
        <div className="flex-1 flex flex-col justify-around w-full max-w-md h-full">
          ...
          <div className="flex justify-center">
            <div ref={dropZoneRef} className={twMerge("w-72 h-72 bg-white rounded-[3rem] border-4 border-dashed flex items-center justify-center transition-all duration-300", isMatched ? "border-green-500 bg-green-50 border-solid" : "border-slate-300")}>
          ...
          <div className="grid grid-cols-3 gap-2 w-full px-1">
            {options.map((item, index) => {
              ...
              // KİLİTLEME: 2. Yanlışta, yanlış şıkları kilitle
                ...
                <div key={index} className="relative flex justify-center items-center h-36">
                  <motion.div
                    ...
                    animate={
                        ...
                        ? { y: [0, -350, -350, 0], scale: [1, 1.2, 1.2, 1], x: 0 } // MODELLEME (HIZLANDI)
                        : { scale: 1, opacity: isLocked ? 0.3 : 1 }
                    }
                    transition={
                        (isModeling && isCorrectItem)
                        ? { duration: 2.5, times: [0, 0.4, 0.7, 1], ease: "easeInOut" } // SÜRE: 2.5 sn
                        : { duration: 0.3 }
                    }
      ...
      <AnimatePresence>
        {showFeedback && (
            <motion.div 
                ...
                className="absolute top-[20%] left-0 right-0 flex justify-center pointer-events-none z-[110]"
            >
                <div className={twMerge(
                    "p-8 rounded-full shadow-2xl border-4 bg-white",
                    showFeedback === 'correct' ? "border-green-500" : "border-red-500"
                )}>
