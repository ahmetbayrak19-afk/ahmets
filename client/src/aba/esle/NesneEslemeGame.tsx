...
export default function NesneEslemeGame({ mode, onClose, onComplete }: GameProps) {
  ...
  const generateQuestion = () => {
    // 1. Rastgele Nesne Seç (Örn: Anahtar)
    const concept = OBJECT_DATA[Math.floor(Math.random() * OBJECT_DATA.length)];
    
    // 2. Bu nesnenin gerçek resimlerinden (real) rastgele BİRİNİ seç.
    // (anahtar.png veya anahtar1.png gelebilir)
    const selectedImg = concept.real[Math.floor(Math.random() * concept.real.length)];

    // 3. Yanıltıcıları Seç (Diğer nesnelerin gerçek resimlerinden)
    const others = OBJECT_DATA.filter(i => i.id !== concept.id).sort(() => 0.5 - Math.random()).slice(0, 2);
    const distractors = others.map(o => o.real[Math.floor(Math.random() * o.real.length)]);

    // 4. Hedef ve Seçenekleri Kur
    setTargetItem({ id: concept.id, name: concept.name, src: selectedImg });
    
    // Doğru cevap BİREBİR AYNI resim
      ...
      {phase === 'playing' && (
        <div className="flex-1 flex flex-col justify-around w-full max-w-md h-full">
          ...
          <div className="flex justify-center">
            <div ref={dropZoneRef} className={twMerge("w-72 h-72 bg-white rounded-[3rem] border-4 border-dashed flex items-center justify-center transition-all", isMatched ? "border-green-500 bg-green-50 scale-105" : "border-slate-300")}>
