...
export default function EslemeGame({ onClose }: { onClose: () => void }) {
  ...
  const generateChunk = (xPos: number) => {
    ...
    // Ot çıkma ihtimalini ve çeşitliliğini ayarlıyoruz
        ...
        >
            ...
            >
                ...
                >
                    ...
                    {/* 3. KATMAN: KUM (ALTZEMİN) - Z-INDEX DÜŞÜK */}
                    {chunks.map(chunk => (
                        <div 
                            key={`base-${chunk.id}`}
                            className="absolute bottom-0 pointer-events-none"
                            style={{
                                left: 0, width: CHUNK_WIDTH, height: ZEMIN_YUKSEKLIK,
                                transform: `translateX(${chunk.x}px)`,
                                zIndex: 10 // Balığın arkasında
                            }}
                        >
                            <div 
                    ...
                    {/* 4. KATMAN: HEDEFLER (YEMLER) - Z-INDEX ORTA */}
                    {targets.map(t => (
                        <div 
                            key={t.id}
                            ...
                            style={{ left: t.x, top: t.y, backgroundColor: t.color, zIndex: 20 }}
                        >
                            <div className="w-4 h-4 bg-white/40 rounded-full blur-sm"></div>
                        </div>
                    ))}

                    {/* 5. KATMAN: BALIK - Z-INDEX ORTA-ÜST */}
                    {isPlaying && (
                        <div 
                            ...
                            style={{
                                ...
                                width: 160, 
                                height: 120,
                                zIndex: 30, // Kumun önünde, otların arkasında
                                transform: (() => {
                    ...
                    {/* 6. KATMAN: OTLAR (USTZEMİN) - Z-INDEX EN YÜKSEK */}
                    {chunks.map(chunk => chunk.overlay && (
                        <div 
                            key={`overlay-${chunk.id}`}
                            className="absolute bottom-0 pointer-events-none"
                            style={{
                                left: 0, width: CHUNK_WIDTH, height: ZEMIN_YUKSEKLIK,
                                transform: `translateX(${chunk.x}px)`,
                                zIndex: 40 // EN ÜST KATMAN (Balığı örter)
                            }}
                        >
                            <div 
                                className="absolute bottom-0 left-0 w-full h-full"
                                style={{
                                    ...
                                    // Yosunlara hafif saydamlık veya karışım modu ekleyerek daha doğal durmasını sağlayabiliriz
