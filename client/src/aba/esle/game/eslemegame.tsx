...
// 🔥 YENİ: Göbeği kameraya çevirme açısı (35 derece)
...
function World({ fishUrl, seaUrl, dracoBase, log }: { fishUrl: string; seaUrl: string; dracoBase: string; log: (m: string) => void; }) {
  ...
  // 🔥 YENİ: Göbek dönüşünü tutacak değişken
  ...
  useFrame((state, dt) => {
    ...
    if (moving) {
      ...
      // ==========================================
      // ✅ SENİN YAZDIĞIN KUSURSUZ SAĞ-SOL KİLİDİ
      // ==========================================
      ...
      // ✅ YUKARI/AŞAĞI BURNU DİKME
      const pitchDirection = (targetYaw > 1.5) ? 1 : -1;
      let targetPitch = ny * MAX_PITCH_ANGLE * pitchDirection;

      // 🔥 YENİ: GÖBEK GÖSTERME MANTIĞI
      // Yukarı çıkarken (ny > 0) balığı kameraya doğru yan yatırıyoruz ki turuncu karın görünsün.
      let targetRoll = ny * BELLY_ROLL_ANGLE * pitchDirection;
      ...
      currentRoll.current = lerp(currentRoll.current, targetRoll, tPitch); // 🔥 YENİ

    } else {
      fishVel.current.multiplyScalar(DRAG_STOP);
      const tPitch = 1 - Math.pow(0.001, dt * TURN_SMOOTH_PITCH);
      currentPitch.current = lerp(currentPitch.current, 0, tPitch);
      currentRoll.current = lerp(currentRoll.current, 0, tPitch); // 🔥 YENİ
    }
    ...
    // ✅ SON VURUŞ: Senin kodun aynısı, sadece araya X ekseni (rotateX) eklendi!
    fishGroup.current.rotation.set(0, currentYaw.current, 0, 'YXZ'); // Sağ-Sol döner
    fishGroup.current.rotateZ(currentPitch.current);                 // Burnunu kaldırır
    fishGroup.current.rotateX(currentRoll.current);                  // 🔥 Göbeği kameraya devirir!
  ...
  return (
    <>
      ...
      <CanvasEvents 
        onDown={() => (dragging.current = true)} 
        onUp={() => (dragging.current = false)} 
      />
    </>
  );
}
...
export default function EslemeGame() {
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", background: BG_COLOR, touchAction: "none" }}>
      <Overlay title="2.5D KUSURSUZ PROFİL & GÖBEK ŞOVU" lines={lines} onClear={clear} />
