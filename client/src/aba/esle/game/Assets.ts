// Assets.ts
// DİKKAT: Resimlerin hepsi bu dosya ile AYNI klasörde olmalı.
...
export const loadAssets = async (): Promise<AssetLibrary> => {
    // Hata korumalı yükleyici
    const loadImage = (src: string, name: string) => new Promise<HTMLImageElement>((resolve) => {
        const img = new Image();
        img.src = src;
        img.onload = () => resolve(img);
        img.onerror = () => {
            console.warn(`UYARI: ${name} yüklenemedi (${src}). Oyun devam ediyor.`);
            resolve(new Image()); // Boş resim dön, oyun patlamasın
        };
    });

    try {
        const [swim, eat, su, gece, zeminler, ustler] = await Promise.all([
            Promise.all(SWIM_SRCS.map((s, i) => loadImage(s, `yuzme_${i}`))),
            Promise.all(EAT_SRCS.map((s, i) => loadImage(s, `yeme_${i}`))),
            loadImage(suDokuImg, 'su_doku'),
            loadImage(geceImg, 'gece'),
            Promise.all([loadImage(altzemin1, 'zemin1'), loadImage(altzemin2, 'zemin2')]),
            Promise.all([loadImage(ustzemin1, 'ust1'), loadImage(ustzemin2, 'ust2'), loadImage(ustzemin3, 'ust3')])
        ]);

        return { swim, eat, su, gece, zeminler, ustler };
    } catch (e) {
        console.error("Asset yükleme hatası:", e);
        throw e;
    }
};
